...
class osnapstream: public std::ostream {
public:
    explicit osnapstream(std::ostream *out,
                         unsigned chunksize = defaultChunkSize);
    void terminate();
private:
    static const unsigned defaultChunkSize = osnapstreambuf::defaultChunkSize;
    osnapstreambuf osbuf_;
};

class isnapstream: public std::istream {
public:
    explicit isnapstream(std::istream *in);
private:
